...
{
	double LAMBDA = 0.01; // 0.03表示每秒30次 ，时间单位换算为毫秒
	...
	{
		return 1 - calc_probability_fault(interference_win, 0);
	}
		...
		int frame_index)
	{
		...
		double prob_unschedulable = calc_probability_fault(EI_LEN + trans_time) * pi_1;
	...
	ProbResult analyze_frame_probability222(
		const CanfdFrame &frame,
		const std::vector<CanfdFrame> &sorted_frames,
		int frame_index)
	{
		...
		while (std::abs(E_R - E_R_PRE) > 0.0001)
		{
			double interference = 0.0;
			for (int j = frame_index - 1; j >= 0; j--)
			{
