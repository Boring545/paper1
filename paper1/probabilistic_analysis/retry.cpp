...
double STM[2][2] = {0.999, 0.001, 0.99, 0.01};
...
double calc_interference() { return 0.0; }
...
class StateNode {
 public:
  ...
  void addChild(std::shared_ptr<StateNode> child) { children.emplace_back(child); }
};
...
ProbResult analyze_frame_probability(const CanfdFrame& frame, const std::vector<CanfdFrame>& sorted_frames,
  ...
  while (!que.empty()) {
    ...
    for (int j = frame_index - 1; j >= 0; j--) {
      interference += std::ceil((t - trans_time + TIME_BIT_ARBITRATION + jitter) / sorted_frames[j].get_period()) *
                      sorted_frames[j].get_trans_time();
    }
...
ProbResult analyze_frame_probability(const CanfdFrame& frame, const std::vector<CanfdFrame>& sorted_frames,
  ...
  while (std::abs(E_R - E_R_PRE) > 0.0000001) {
    double interference = 0.0;
    for (int j = frame_index - 1; j >= 0; j--) {
      interference += std::ceil((E_R - trans_time + TIME_BIT_ARBITRATION + jitter) / sorted_frames[j].get_period()) *
                      sorted_frames[j].get_trans_time();
    }
    E_R_PRE = E_R;
...
struct bak_msg_rem {
  ...
  // 带参构造函数
  bak_msg_rem(const Message& msg, double prob) : backup_msg(msg), remain_prob(prob) {}
};
// 尝试在高优先级帧中插入备份
int try_insert_backups(PackingScheme& scheme, std::vector<bak_msg_rem>& bak_msgs, CanfdFrameVec& sorted_frames,
                       std::unordered_map<FrameId, ProbResult>& result, const CanfdFrame& frame, size_t frame_index) {
  int cnt = 0;
  for (int k = 0; k < bak_msgs.size(); k++) {
    ...
    for (int j = 0; j < frame_index; j++) {
      ...
      if (frame.ecu_pair == hp_frame.ecu_pair && m.get_period() % hp_frame.get_period() == 0 &&
          m.get_data_size() <= hp_frame.get_free_size()) {
...
void save_prob_result(const std::string& address, std::unordered_map<MessageCode, ProbData>& signal_p_timeout,
                      double b_u, bool append = false) {
...
void new_analysis(PackingScheme& scheme, int enable_backup, std::string timestamp) {
  ...
  std::sort(sorted_frames.begin(), sorted_frames.end(), [](const CanfdFrame& f1, const CanfdFrame& f2) {
    if (f1.get_offset() != f2.get_offset()) return f1.get_offset() < f2.get_offset();
...
std::unordered_map<MessageCode, ProbData> probabilistic_analysis(PackingScheme& scheme, int enable_backup,
  ...
  for (MessageID index = 0; index < MESSAGE_INFO_VEC.size(); index++) {
    ...
    signal_p_timeout[code] = {
        index,           msg_info.period, msg_info.level, msg_info.type, THRESHOLD_RELIABILITY[msg_info.level],
  ...
  std::sort(sorted_frames.begin(), sorted_frames.end(),
            [](const CanfdFrame& f1, const CanfdFrame& f2) { return f1.get_priority() < f2.get_priority(); });
  ...
  for (size_t i = 0; i < sorted_frames.size(); i++) {
    ...
    // 分析当前帧
    ProbResult res = analyze_frame_probability(frame, sorted_frames, max_fault_rate, i, COST_MAX_ERROR_FRAME);
      ...
      if (it == signal_p_timeout.end()) {
        signal_p_timeout[code] = {
            ...
            prob_res.p_timeout  // 信号的故障概率 = 报文的故障概率 只在信号无offset，信号周期等于deadline时
                                // TODO
                                // frame的不可调度概率不完全等于所装载的每个帧的不可调度概率，但理论上，同周期的信号，deadline和帧deadline一致，那些周期为帧周期倍数的信号，是特殊点
        };
      } else {
  ...
  const std::string address_origin =
      "D:/document/CODE/paper1/storage/signal_fault_probabilities_origin_" + timestamp + ".txt";
  const std::string address_repack =
      "D:/document/CODE/paper1/storage/signal_fault_probabilities_repack_" + timestamp + ".txt";
#else
