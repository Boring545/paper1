...
constexpr int OPTION_CANFD_PAYLOAD_SIZE[] = {0, 1,  2,  3,  4,  5,  6,  7,
                                             8, 12, 16, 20, 24, 32, 48, 64};  // CANFD帧payload可选长度，单位为byte
...
class EcuPair {
 public:
  ...
  EcuPair(int src = 0, int dst = 0) : src_ecu(src), dst_ecu(dst) {}
  bool operator==(const EcuPair& other) const { return (src_ecu == other.src_ecu) && (dst_ecu == other.dst_ecu); }
 ...
 public:
  ...
  MessageInfo(const MessageInfo& minfo, int new_src_ecu, int _type = 1)
      : code(minfo.code),
        ...
        ecu_pair(new_src_ecu, minfo.ecu_pair.dst_ecu),
        offset(minfo.offset),
        level(minfo.level),
        type(_type) {}
...
class Message {
 ...
 public:
  ...
  void assign_frame(FrameId fid) { this->frame_index = fid; }
  void clear_frame() { this->frame_index = -1; }
...
class CanfdFrame {
 private:
  ...
  int payload_size =
      0;  // payload尺寸和数据尺寸不完全一样，取值有0, 1, 2, 3, 4, 5, 6, 7, 8, 12, 16, 20, 24, 32, 48 or 64 bytes
 ...
 private:
  // 将数据尺寸（单位b）转换为合适的payload尺寸（单位byte），payload取值有0, 1, 2, 3, 4, 5, 6, 7, 8, 12, 16, 20, 24, 32,
  // 48 or 64 bytes
 ...
 public:
  ...
  int get_free_size() { return payload_size * 8 - data_size; }
  ...
  bool empty() const { return this->msg_set.empty(); }
  ...
  int get_period() const { return this->period; }
  int get_paylaod_size() const { return this->payload_size; }
  ...
  CanfdFrame(FrameId _id, int _period, int _deadline, const EcuPair& _ecu_pair, double _offset) {
    this->id = _id;
    this->period = _period;
  ...
  CanfdFrame(FrameId _id) { this->id = _id; }
  CanfdFrame() { return; }
