...
namespace cfd::schedule {
	bool feasibility_check(CanfdFrameMap& frame_map)
	{
		...
		{
			CanfdFrame& frame = *sorted_frames[frame_index];
			...
			double E_R = max_lp_trans_time + trans_time; // 初值建议含 blocking（收敛更快）
			double E_R_PRE = -1.0;
			...
			for (int iter = 0; iter < 1000 && std::abs(E_R - E_R_PRE) > EPS; ++iter)
			{
				...
				// 提前剪枝：一旦超过 deadline，可直接判失败
				if (E_R > deadline + EPS) {
					DEBUG_MSG_DEBUG2(std::cout,
						"[Sched FAIL] frame=", frame.get_id(),
						" pri=", frame.get_priority(),
						" R=", E_R, " D=", deadline,
						" B=", max_lp_trans_time, " C=", trans_time
					);
					return false;
				}
			}
			...
			// 最终再判断一次（理论上上面剪枝已经覆盖）
			if (E_R > deadline + EPS) return false;
		}
	...
	bool assign_priority_by_period(CanfdFrameMap& frame_map)
	{
		...
		for (auto& fp : frame_map) {
			auto& frame = fp.second;
			if (frame.empty()) continue;
			DEBUG_MSG_DEBUG2(std::cout, "任务: ", frame.get_id(),
				" 周期: ", frame.get_period(),
				" 截止期: ", frame.get_deadline(),
				" 优先级: ", frame.get_priority());
		}
		DEBUG_MSG_DEBUG2(std::cout, "=============================== ");
		return true;
	}
